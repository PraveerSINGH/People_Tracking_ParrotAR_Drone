...
Matrix Matrix::getMat(int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  ...
  if (i1<0 || i2>=m || j1<0 || j2>=n || i2<i1 || j2<j1) {
    ostringstream msg;
    msg << "ERROR: Cannot get submatrix [" << i1 << ".." << i2 <<
        "] x [" << j1 << ".." << j2 << "]" <<
...
void Matrix::setMat(const Matrix &M,const int32_t i1,const int32_t j1) {
  if (i1<0 || j1<0 || i1+M.m>m || j1+M.n>n) {
    ostringstream msg;
    msg << "ERROR: Cannot set submatrix [" << i1 << ".." << i1+M.m-1 <<
        "] x [" << j1 << ".." << j1+M.n-1 << "]" <<
        " of a (" << m << "x" << n << ") matrix." << endl;
    throw runtime_error(msg.str());
  }
...
void Matrix::setVal(FLOAT s,int32_t i1,int32_t j1,int32_t i2,int32_t j2) {
  ...
  if (i2<i1 || j2<j1) {
    ostringstream msg;
    msg << "ERROR in setVal: Indices must be ordered (i1<=i2, j1<=j2)." << endl;
    throw runtime_error(msg.str());
  }
  for (int32_t i=i1; i<=i2; i++)
    for (int32_t j=j1; j<=j2; j++)
...
Matrix Matrix::diag (const Matrix &M) {
  ...
  ostringstream msg;
  msg << "ERROR: Trying to create diagonal matrix from vector of size (" << M.m << "x" << M.n << ")" << endl;
  throw runtime_error(msg.str());
}
...
Matrix Matrix::reshape(const Matrix &M,int32_t m_,int32_t n_) {
  if (M.m*M.n != m_*n_) {
    ostringstream msg;
    msg << "ERROR: Trying to reshape a matrix of size (" << M.m << "x" << M.n <<
...
Matrix Matrix::operator+ (const Matrix &M) {
  ...
  if (A.m!=B.m || A.n!=B.n) {
    ostringstream msg;
    msg << "ERROR: Trying to add matrices of size (" << A.m << "x" << A.n <<
        ") and (" << B.m << "x" << B.n << ")" << endl;
    throw runtime_error(msg.str());
  }
  Matrix C(A.m,A.n);
...
Matrix Matrix::operator- (const Matrix &M) {
  ...
  if (A.m!=B.m || A.n!=B.n) {
    ostringstream msg;
    msg << "ERROR: Trying to subtract matrices of size (" << A.m << "x" << A.n <<
        ") and (" << B.m << "x" << B.n << ")" << endl;
    throw runtime_error(msg.str());
  }
  Matrix C(A.m,A.n);
...
Matrix Matrix::operator* (const Matrix &M) {
  ...
  if (A.n!=B.m) {
    ostringstream msg;
    msg << "ERROR: Trying to multiply matrices of size (" << A.m << "x" << A.n <<
        ") and (" << B.m << "x" << B.n << ")" << endl;
    throw runtime_error(msg.str());
  }
  Matrix C(A.m,B.n);
...
Matrix Matrix::operator/ (const Matrix &M) {
  ...
  if (A.m==B.m && A.n==B.n) {
    ...
    ostringstream msg;
    msg << "ERROR: Trying to divide matrices of size (" << A.m << "x" << A.n <<
        ") and (" << B.m << "x" << B.n << ")" << endl;
    throw runtime_error(msg.str());
  } 
}
...
Matrix Matrix::operator/ (const FLOAT &s) {
  if (fabs(s)<1e-20) {
    ostringstream msg;
    msg << "ERROR: Trying to divide by zero!" << endl;
    throw runtime_error(msg.str());
  }
  Matrix C(m,n);
  for (int32_t i=0; i<m; i++)
...
Matrix Matrix::cross (const Matrix &a, const Matrix &b) {
  if (a.m!=3 || a.n!=1 || b.m!=3 || b.n!=1) {
    ostringstream msg;
    msg << "ERROR: Cross product vectors must be of size (3x1)" << endl;
    throw runtime_error(msg.str());
  }
  Matrix c(3,1);
...
Matrix Matrix::inv (const Matrix &M) {
  if (M.m!=M.n) {
    ostringstream msg;
    msg << "ERROR: Trying to invert matrix of size (" << M.m << "x" << M.n << ")" << endl;
    throw runtime_error(msg.str());
  }
  Matrix A(M);
  Matrix B = eye(M.m);
...
bool Matrix::inv () {
  if (m!=n) {
    ostringstream msg;
    msg << "ERROR: Trying to invert matrix of size (" << m << "x" << n << ")" << endl;
    throw runtime_error(msg.str());
  }
  Matrix A(*this);
  eye();
...
FLOAT Matrix::det () {
  ...
  if (m != n) {
    ostringstream msg;
    msg << "ERROR: Trying to compute determinant of a matrix of size (" << m << "x" << n << ")" << endl;
    throw runtime_error(msg.str());
  }
...
bool Matrix::solve (const Matrix &M, FLOAT eps) {
  ...
  if (A.m != A.n || A.m != B.m || A.m<1 || B.n<1) {
    ostringstream msg;
    msg << "ERROR: Trying to eliminate matrices of size (" << A.m << "x" << A.n <<
            ") and (" << B.m << "x" << B.n << ")" << endl;
    throw runtime_error(msg.str());
  }
  ...
  // loop variables
  int32_t i, icol=0x7FFFFFFF, irow=0x7FFFFFFF, j, k, l, ll;
...
bool Matrix::lu(int32_t *idx, FLOAT &d, FLOAT eps) {
  ...
  if (m != n) {
    ostringstream msg;
    msg << "ERROR: Trying to LU decompose a matrix of size (" << m << "x" << n << ")" << endl;
    throw runtime_error(msg.str());
  }
  
  int32_t i,imax=0x7FFFFFFF,j,k;
...
void Matrix::svd(Matrix &U2,Matrix &W,Matrix &V) {
  ...
  int32_t flag,i,its,j,jj,k,l=0x7FFFFFFF,nm=0x7FFFFFFF;
