...
//  printf("VOM::eM(): %ld matches as input\n", p_matched.size());
  ...
  if (!normalizeFeaturePoints(p_matched_normalized,Tp,Tc)) {
    printf("VOM::eM(): aborted at normalizeFeaturePoints()\n");
    return vector<double>();
  }
...
//  printf("VOM::eM(): %ld inliers\n", inliers.size());
  
  // are there enough inliers?
  if (inliers.size()<10) {
    printf("VOM::eM(): aborted because too few inliers (%ld, but 10 required)\n", inliers.size());
    return vector<double>();
  }
...
//  printf("VOM::eM(): %d initial 3D points\n", X.n);
...
//  printf("VOM::eM(): %d normalized 3D points in front of camera\n", X_plane.n);
  
  // we need at least 10 points to proceed
  if (X_plane.n<10) {
    printf("VOM::eM(): aborted because too few normalized 3D points (%d, but 10 required)\n", X_plane.n);
    return vector<double>();
  }
  
  // get the median 3D distance (The set of elements closer than it is the function's return value, which is ignored here.)
  double median;
  smallerThanMedian(X_plane,median);
  
//  printf("VOM::eM(): median = %f\n", median);
  
  // return error on large median (little motion)
  if (median>param.motion_threshold) {
//    printf("VOM::eM(): aborted because too big median (%f, but %f required)\n", median, param.motion_threshold);
    return vector<double>();
  }
  
  // project features to 2d
  Matrix x_plane(3,X_plane.n);
  x_plane.setMat(X_plane.getMat(0,0,2,-1),0,0);
  
  Matrix n(3,1);
  n.val[0][0]       =  cos(param.pitch)*sin(param.roll);
  n.val[1][0]       =  cos(param.pitch)*cos(param.roll);
  n.val[2][0]       = -sin(param.pitch);
  Matrix   d        = ~n*x_plane;
...
//  printf("VOM::eM(): tested %ld out of %d seed points for ground plane\n", counterSums, x_plane.n);
  
  if(best_sum == 0) {
    printf("VOM::eM(): aborted because no point has been a suitable seed for ground plane\n");
    return vector<double>();
  }
  
  t = t*param.height/d.val[0][best_idx];
  points3D = X.getMat(0,0,2,-1)*param.height/d.val[0][best_idx];
  
//  printf("VOM::eM(): succeeded\n");
