...
namespace {
	...
	bool findLine( const cv::Mat  imageBottom,
		...
		cv::HoughLines( red, lines, 1, CV_PI / 180, 100 * scaleFactor * scaleFactor ) ;
		if( !lines.empty() ) {
...
void DroneAppFollowLine::processImageBottom( const cv_bridge::CvImage imageBottom ) {
	...
	if( found ) {
		...
		capacity -= pow( ( movement.linear.z  > 0.0 ?
		                   movement.linear.z  / this->commands.controllerLinZ.uMax :
		                   movement.linear.z  / this->commands.controllerLinZ.uMin ), 2 ) ;
		...
		movement.linear.x = sqrt( capacity ) * this->commands.controllerLinX.uMax ;
		
		// further reduce forward motion if fling away from line
